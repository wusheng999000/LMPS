...
void CFitting3D::setinputcloud(PointCloud<PointT>::Ptr input_cloud)
{
	cloud = input_cloud;
	getMinMax3D(*input_cloud, point_min, point_max);
}
void CFitting3D::grid_mean_xyz(double x_resolution, double y_resolution, vector<double>&x_mean, vector<double> &y_mean, vector<double>&z_mean, PointCloud<PointT>::Ptr &new_cloud)
{
	if (y_resolution<=0)
	{
...
void CFitting3D::line_fitting(vector<double>x, vector<double>y, double &k, double &b){
...
void CFitting3D::polynomial2D_fitting(vector<double>x, vector<double>y, double &a, double &b, double &c)
{
...
void CFitting3D::polynomial3D_fitting(vector<double>x, vector<double>y, vector<double>z, double &a, double &b, double &c)
{
	int num_point = x.size();
	MatrixXd A_(3, 3), B_(3, 1), A123(3, 1);
	double A01(0.0), A02(0.0), A12(0.0), A22(0.0), B00(0.0), B10(0.0), B12(0.0);
...
void CFitting3D::polynomial3D_fitting_display(double step_)
{
	...
	//visualization::PCLVisualizer::Ptr viewer(new visualization::PCLVisualizer(L"最后拟合的多项式曲线"));
	view->addModelFromPolyData(polyData, "1");
	//*******************************************
	...
	view->addPointCloud(tem_point, color1, "point1");
	view->setPointCloudRenderingProperties(visualization::PCL_VISUALIZER_POINT_SIZE, 3, "point1");
	...
	view->addPointCloud(tem_point1, "orginal");
	view->setPointCloudRenderingProperties(visualization::PCL_VISUALIZER_POINT_SIZE, 2, "orginal");
	//显示端点
	...
	view->addPointCloud(duandian_point, color2, "duandian");
	view->setPointCloudRenderingProperties(visualization::PCL_VISUALIZER_POINT_SIZE, 5, "duandian");
